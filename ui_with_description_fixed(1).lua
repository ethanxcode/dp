...
local Library = { } do
    ...
    Library.SubTab = function(Self, Params)
        ...
        local function ForEachSectionPart(Section, Handler)
            ...
            local Objects = {
                ...
                Parts.Description and Parts.Description.Instance,
        ...
        local function LayoutPage()
            for _, Column in SubTab.Columns do
                for _, Section in Column.Sections do
                    ...
                    Section.Items.Frame.Instance.Position = UDim2.fromOffset(0, 0)

                    if Section.Items.Description then
                        Section.Items.Description.Instance.Visible = Section.Description ~= ""
                    end
    ...
    Library.Section = function(Self, Params)
        ...
        local DescriptionHeight = Section.Description ~= ""
            and math.max(16, math.ceil(DescriptionBounds.Y))
            or 0

        local ContentTop = Section.Description ~= ""
            and (38 + DescriptionHeight)
            or 36

        Section.HeaderHeight = ContentTop

        -- One unified card: title, description and rows all live inside it.
        Items.Frame = MakeFrame({
            Parent = Items.Holder.Instance,
            Pos = UDim2.fromOffset(0, 0),
            Size = UDim2.fromOffset(Column.Width, 60),
            Color = "Section",
            Round = 10,
            Z = 3
        })
        ...
        if Section.Description ~= "" then
            Items.Description = MakeText({
                ...
                TextSize = 13,
                Pos = UDim2.fromOffset(13, 28),
                Size = UDim2.new(1, -26, 0, DescriptionHeight),
                Color = "DimText",
                Align = Enum.TextXAlignment.Center,
                Wrap = true,
                Z = 4
            ...
            Items.Description.Instance.TextYAlignment = Enum.TextYAlignment.Top
        end
        ...
        local function SyncHeader()
            ...
            Items.Label.Instance.Size = UDim2.new(1, -26, 0, 20)
        end

        local function SyncDescription()
            if not Items.Description then
                return
            end

            DescriptionBounds = MeasureText(
                Items.Description.Instance.Text,
                13,
                math.max(DescriptionW, 40),
                UiFont
            )

            DescriptionHeight = math.max(16, math.ceil(DescriptionBounds.Y))
            ContentTop = 38 + DescriptionHeight
            Section.HeaderHeight = ContentTop

            Items.Description.Instance.Size = UDim2.new(
                1, -26,
                0,
                DescriptionHeight
            )
        ...
        Library:Connect(
            Items.Label.Instance:GetPropertyChangedSignal("TextBounds"),
            SyncHeader
        )

        if Items.Description then
            Library:Connect(
                Items.Description.Instance:GetPropertyChangedSignal("TextBounds"),
                SyncDescription
            )
        end
        ...
        function Section:Reflow()
            ...
            local FrameHeight = Visible > 0
                and (Y + 8)
                or (ContentTop + 8)

            Section.HeaderHeight = ContentTop

            Items.Frame.Instance.Position = UDim2.fromOffset(0, 0)
            Items.Frame.Instance.Size = UDim2.fromOffset(
                Section.Width,
                FrameHeight
            )

            Items.Holder.Instance.Visible = Visible > 0
            Section.Height = Visible > 0 and FrameHeight or 0

            Items.Holder.Instance.Size = UDim2.fromOffset(
                Section.Width,
                math.max(Section.Height, 1)
            )

            Column:Reflow()
        end

        function Section:SetDescription(Text)
            Text = tostring(Text or "")
            Section.Description = Text

            if not Items.Description and Text ~= "" then
                Items.Description = MakeText({
                    Parent = Items.Frame.Instance,
                    Text = Text,
                    TextSize = 13,
            ...
            elseif Items.Description then
                Items.Description.Instance.Text = Text
                Items.Description.Instance.Visible = Text ~= ""
            end

            DescriptionBounds = Text ~= ""
                and MeasureText(Text, 13, math.max(DescriptionW, 40), UiFont)
                or Vector2.new(0, 0)
