...
local Library = { } do
        ...
        local function LayoutPage()
            for _, Column in SubTab.Columns do
                for _, Section in Column.Sections do
                    ...
                    Section.Items.Frame.Instance.Position = UDim2.fromOffset(
                        0,
                        Section.HeaderHeight or 26
                    )
    ...
    Library.Section = function(Self, Params)
        ...
        local DescriptionW = math.max(Column.Width - 28, 40)
        local DescriptionBounds = Section.Description ~= ""
        ...
        local function SyncDescription()
            if not Items.Description then return end
            ...
            Items.Description.Instance.Size = UDim2.fromOffset(
                DescriptionW,
                math.max(0, math.ceil(Bounds.Y))
            )
        ...
        Items.Frame = MakeFrame({
            ...
            Pos = UDim2.fromOffset(
                0,
                Section.Description ~= "" and (34 + math.ceil(DescriptionBounds.Y)) or 26
            ),
        ...
        function Section:Reflow()
            ...
            Items.Frame.Instance.Position = UDim2.fromOffset(0, Section.HeaderHeight)
            Items.Frame.Instance.Size = UDim2.fromOffset(Section.Width, FrameHeight)
            Items.Holder.Instance.Visible = Visible > 0
            Section.Height = Visible > 0 and (Section.HeaderHeight + FrameHeight) or 0
        ...
        function Section:SetDescription(Text)
            ...
            if not Items.Description and Text ~= "" then
                Items.Description = MakeText({
                    ...
                    Size = UDim2.fromOffset(DescriptionW, 16),
                    Color = "DimText",
                    Wrap = true,
                    Z = 4
                })
            ...
            if Items.Description then
                DescriptionBounds = Text ~= ""
                    and MeasureText(Text, 13, DescriptionW, UiFont)
                    or Vector2.new(0, 0)

                Section.HeaderHeight = Text ~= ""
                    and (34 + math.ceil(DescriptionBounds.Y))
                    or 26
                ...
                Items.Description.Instance.Size = UDim2.fromOffset(
                    DescriptionW,
                    math.max(0, math.ceil(DescriptionBounds.Y))
                )
            end
