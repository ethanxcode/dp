...
local Library = { } do
    ...
    Library.Window = function(Self, Params)
        ...
        RedBtn:OnHover(function()
            ...
            RedBtn:Tween({ BackgroundColor3 = Color3.fromRGB(255, 95, 86) })
        end)

        YellowBtn:OnHover(function()
        ...
        local LogoBg = MakeFrame({
            Parent = LogoHolder.Instance,
            Pos = UDim2.fromOffset(24, 24),
        ...
        MakeAccentShadow(LogoBg.Instance, UDim2.fromOffset(0, 0), UDim.new(0, 16), 0.2)

        MakeImage({
            Parent = LogoBg.Instance,
            Icon = Window.Icon,
            Anchor = Vector2.new(0.5, 0.5),
            Pos = UDim2.new(0.5, 0, 0.5, 0),
        ...
        local LogoHit = MakeButton({ Parent = LogoBg.Instance, Z = 62 })

        LogoBg:OnHover(function()
            LogoBg:Tween({ BackgroundColor3 = Library.Theme.Hover })
        end, function()
            LogoBg:Tween({ BackgroundColor3 = Library.Theme.Section })
        end)

        do
            local LInst = LogoBg.Instance
            local Dragging, DragStart, StartPos = false, nil, nil

            Library:Connect(LInst.InputBegan, function(Input)
                if Input.UserInputType ~= Enum.UserInputType.MouseButton1
                and Input.UserInputType ~= Enum.UserInputType.Touch then return end
                Dragging = true
                DragStart = Input.Position
                StartPos = Vector2.new(LInst.Position.X.Offset, LInst.Position.Y.Offset)
            ...
            Library:Connect(UserInputService.InputChanged, function(Input)
                ...
                LInst.Position = UDim2.fromOffset(
                    math.clamp(StartPos.X + Delta.X, 0, Vp.X - 54),
                    math.clamp(StartPos.Y + Delta.Y, 0, Vp.Y - 54)
                )
            end)
        end
        ...
        local function SetMinimized(Bool)
            if WindowMinimized == Bool then return end
            WindowMinimized = Bool
            Library:CloseAllPopups()

            if Bool then
                Items.Root:FadeDescendants(false, function()
                    Items.Root.Instance.Visible = false
                end)
                task.delay(Library.Animation.Time + 0.06, function()
                    LogoHolder.Instance.Enabled = true
                    LogoBg:FadeDescendants(true)
                end)
            else
                LogoBg:FadeDescendants(false, function()
                    LogoHolder.Instance.Enabled = false
                end)
                task.delay(Library.Animation.Time + 0.06, function()
        ...
        RedHit:Connect("MouseButton1Down", function()
            Window:SetOpen(false)
            task.delay(Library.Animation.Time + 0.12, function()
                pcall(function() LogoHolder.Instance:Destroy() end)
                Library:Unload()
            end)
        end)
        ...
        local _BaseUnload = Library.Unload
        Library.Unload = function(Self)
            pcall(function() LogoHolder.Instance:Destroy() end)
            _BaseUnload(Self)
        end
